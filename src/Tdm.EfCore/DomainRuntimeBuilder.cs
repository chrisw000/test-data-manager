        ...
        IReadOnlyList<Assembly> assemblies, ILogger? logger = null)
    {
        ...
        foreach (var contextType in contextTypes)
        {
            ...
            foreach (var efType in context.Model.GetEntityTypes())
            {
                ...
                var repository = RepositoryBinding.Find(logicalName, clrType, profile, assemblies);
                if (repository is null && domain.Persistence != PersistenceMode.DbContextOnly)
                    warnings.Add($"{domain.Name}.{logicalName}: no {NameMatcher.Expand(profile.RepositoryPattern, logicalName)} found — DbContext persistence will be used.");
                else if (repository is { AddMethod: null } && domain.Persistence != PersistenceMode.DbContextOnly)
                    warnings.Add($"{domain.Name}.{logicalName}: {repository.InterfaceType.Name} exposes no recognised persist method — DbContext fallback.");
        ...
        return new DomainRuntime(domain, root, assemblies, contextTypes, bindings, warnings, logger);
    }
        ...
        TdmSettings root, IReadOnlyList<Assembly> assemblies, List<EntityBinding> bindings, List<string> warnings)
    {
        ...
        foreach (var type in assemblies.SelectMany(DbContextActivator.SafeGetTypes))
        {
            ...
            bindings.Add(new EntityBinding
            {
                ...
                ContextType = null,
                EfType = null,
                Repository = RepositoryBinding.Find(logicalName, type, profile, assemblies),
